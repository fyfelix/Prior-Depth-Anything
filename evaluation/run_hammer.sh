...
usage() {
    cat <<'EOF'
Usage:
  bash evaluation/run_hammer.sh [priorda_ckpt=ckpts/prior_depth_anything_vitb_1_1.pth] [mde_ckpt=ckpts/depth_anything_v2_vitl.pth] [camera_type=d435] [frozen_size=vitl] [conditioned_size=vitb] [version=1.1] [cleanup_npy=false]
...
case "${camera_type}" in
    d435|l515|tof) ;;
    *)
        echo "camera_type must be one of: d435, l515, tof; got: ${camera_type}" >&2
        exit 2
        ;;
esac
